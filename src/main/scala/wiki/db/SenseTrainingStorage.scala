...
object SenseTrainingStorage {
  ...
  /**
    * Write a SenseFeatures object to the training tables. This includes the
    * context data and all training examples.
    *
  ...
  def write(input: SenseFeatures): Unit = {
    DB.localTx { implicit session =>
      ...
      val contextId = writeContext(input.page.id, input.context)

      // Then insert all training examples with the context ID
      input.examples.foreach { example =>
        writeTrainingExample(input.page.id, contextId, example)
      }
    }
  }

  /**
    * Write context data to the training_context and training_context_page tables.
    *
    ...
    */
  private def writeContext(sensePageId: Int, context: Context)(implicit session: DBSession): Long = {
    // Insert the context record
    val contextId = sql"""INSERT INTO $contextTable
         (sense_page_id, quality)
         VALUES ($sensePageId, ${context.quality})
         """
  ...
  /**
    * Write a training example to the training_example table.
    *
  ...
  private def writeTrainingExample(
    ...
    sql"""INSERT INTO $exampleTable
         (sense_page_id, context_id, source_page_id, link_destination, label,
          sense_id, commonness, relatedness, context_quality, is_correct_sense)
         VALUES ($sensePageId, $contextId, ${example.sourcePageId}, ${example.linkDestination},
